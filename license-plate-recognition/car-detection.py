...
gray = cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY) #convert to grey scale

gray = cv2.bilateralFilter(gray, 13, 50, 50)
edged = cv2.Canny(gray, 80, 80) #Perform Edge detection
...
img_contours = np.zeros(cropped.shape)
cv2.drawContours(img_contours, contours, -1, (0,255,0), 3)

cv2.imshow('contours', img_contours)
cv2.waitKey(0)

img_contour = np.zeros(cropped.shape)
if screenCnt.any():
